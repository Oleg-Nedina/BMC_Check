...
int bmc_rx_filter_main(struct xdp_md *ctx)
{
	...
	if (dport == htons(11211) && payload+4 <= data_end) {
		...
		if (ip->protocol == IPPROTO_UDP && payload[0] == 'g' && payload[1] == 'e' && payload[2] == 't' && payload[3] == ' ') { // is this a GET request
			...
			unsigned int off;
			/* Load a fresh data_end pointer specifically for the whitespace-skip
			 * loop. The function-level data_end register was already consumed
			 * by prior pointer arithmetic (producing a plain integer), so the
			 * compiler would not generate a packet-pointer comparison for it
			 * inside the loop. A single fresh load before the loop avoids both
			 * problems: the compiler cannot precompute data_end-payload as an
			 * integer (fresh register, only one use-site), and the load is not
			 * repeated per-iteration so the verifier does not undergo state
			 * explosion. */
			void *ws_data_end = (void *)(long)ctx->data_end;
#pragma clang loop unroll(disable)
			for (off = 4; off < BMC_MAX_PACKET_LENGTH; off++) {
				char *p = payload + off;
				if ((void *)(p + 1) > ws_data_end)
					break;
				if (*p != ' ')
					break;
			} // move offset past any extra spaces after 'get '
